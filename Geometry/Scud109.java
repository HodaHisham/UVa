...
public class Scud109 {
	public static void main(String[] args) throws IOException   {
		...
		while(true)
		{
			...
			for (int i = 0; i < n; i++) {
				po[i] = new Point(bf.nextInt(), bf.nextInt());

			}
			p.add(Polygon.convexHull(po));
		}
		double area = 0.0;
		boolean [] b = new boolean[p.size()];
		while(bf.ready()){
			Point q = new Point(bf.nextInt(), bf.nextInt());
			for (int i = 0; i < p.size(); i++) {
				Point [] r = Arrays.copyOf(p.get(i).g,p.get(i).g.length+1);
				r[r.length-1] = r[0];
				Polygon tmp = new Polygon(r);
				if(tmp.inside(q) && !b[i]){
					area += tmp.area();
					b[i] = true;
				}
