...
public class Points478 {
	...
	public static void main(String[] args) throws Exception {
		...
		while(true){
			...
			case 'r':{
				...
				f.add(new Rectangle(new Point(p.x,q.y),new Point(q.x,p.y) ));
				break;
			}
			case 'c':{
				f.add(new Circle(new Point(bf.nextDouble(),bf.nextDouble()), bf.nextDouble()));
				break;
			}
			case 't':{

				f.add(new Triangle(new Point(bf.nextDouble(),bf.nextDouble()), new Point(bf.nextDouble(),bf.nextDouble()),new Point(bf.nextDouble(),bf.nextDouble())));
				break;
			}
			}
		}
		...
		while(true){
			String s = bf.next();
			String s2 = bf.next();
			if(s.equals("9999.9") && s.equals("9999.9"))
				break;
			Point p = new Point(Double.parseDouble(s), Double.parseDouble(s2));
			boolean flag = false;
			for (int i = 0; i < f.size(); i++)
			{
				if(f.get(i).contains(p)){
	...
	abstract static class Figure{
		...
		abstract boolean contains(Point g);
	}
	...
	//
	//    }
	static class Circle extends Figure{ 	//equation: (x-c.x)^2 + (y-c.y)^2 = r^2
		...
		boolean contains(Point p)	//1 for inside, 0 for border, -1 for outside
		{
			double d = p.dist(c);
	...
	static class Triangle extends Figure{
		...
		@Override
		boolean contains(Point g)
		{
			Triangle t1 = new Triangle(a, b, g);
			Triangle t2 = new Triangle(b, c, g);
			Triangle t3 = new Triangle(a, c, g);
			//	    System.out.println(t1.area2() + " "+ t2.area2()+ " "+ t3.area2());
			return Math.abs((t1.area2()+ t2.area2()+ t3.area2()) -area2()) < EPS && !g.onLine(a, b) && !g.onLine(b, c) && !g.onLine(a, c);
		}
	...
	static class Polygon extends Figure{ 
		...
		boolean contains(Point p)	//for convex/concave polygons - winding number algorithm 
		{
			double sum = 0.0;
			for(int i = 0; i < g.length - 1; ++i)
			{
