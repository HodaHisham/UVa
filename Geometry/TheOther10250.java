...
public class TheOther10250 {
	...
	while(bf.ready()){
	   double x1 = bf.nextDouble();
	   double y1 = bf.nextDouble();
	   double x2 = bf.nextDouble();
	   double y2 = bf.nextDouble();
	   Point m = new Point((x1+x2)/2.0,(y1+y2)/2.0);
	   Vector v = new Vector(new Point(x1,y1),new Point(x2,y2)).scale(0.5);
	   Vector f = new Vector(-v.y,v.x);
	   Vector g = new Vector(v.y,-v.x);
	   Point b  = m.translate(f);
	   Point d  = m.translate(g);
