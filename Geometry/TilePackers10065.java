...
public class TilePackers10065 {
    public static double round(double x) {	return Math.round(x * 100) / 100.0; }  //use it because of -0.000
	...
	while(true){
	    ...
	    double area = Polygon.convexHull(Arrays.copyOf(points, n)).area();
	    double a = p.area();
	    double res = (area-a)*100.0/area;
	    out.println("Tile #" + cases++);
	    out.printf("Wasted Space = %.2f ", round(res));
	    out.println("%\n");
	}
	out.flush();
	out.close();
